...
from kis_utils import KoreaInvestEnv, KoreaInvestAPI
from Crypto.Cipher import AES
from base64 import b64decodes
...
class KISAPIForm(QMainWindow, form_class):
  def __init__(
    ...
    # 토큰 자동 갱신 타이머 (1시간마다 체크)
    self.timer_token = QTimer()
    self.timer_token.timeout.connect(self.check_token_renewal)
    self.timer_token.start(3600000)  # 1시간 = 3,600,000ms
    
  def req_ranking(self):
    self.tr_req_queue.put(dict(action_id='등락률상위'))
  
  def check_token_renewal(self):
    """토큰 자동 갱신 체크 (1시간마다 실행)"""
    try:
        logger.info("=== 토큰 자동 갱신 체크 시작 ===")
        
        # TokenManager를 통해 토큰 만료 여부 확인
        from kis_utils import TokenManager
        token_manager = TokenManager()
        
        if token_manager.is_token_expired(hours_threshold=12):
            logger.warning("토큰이 만료되었습니다. 자동 갱신을 시작합니다.")
            
            # 기존 KoreaInvestAPI 인스턴스가 있으면 토큰 갱신
            if hasattr(self, 'korea_invest_api') and self.korea_invest_api:
                # 토큰 갱신 시도
                self.korea_invest_api.initialize_tokens()
                logger.info("토큰 자동 갱신이 완료되었습니다.")
                
                # UI에 갱신 알림
                self.update_status_message("토큰이 자동으로 갱신되었습니다.")
            else:
                logger.warning("KoreaInvestAPI 인스턴스를 찾을 수 없어 토큰 갱신을 건너뜁니다.")
        else:
            logger.info("토큰이 아직 유효합니다.")
            
    except Exception as e:
        logger.error(f"토큰 자동 갱신 체크 중 오류 발생: {e}")
        self.update_status_message(f"토큰 갱신 체크 오류: {str(e)}")
        
    finally:
        logger.info("=== 토큰 자동 갱신 체크 완료 ===")
