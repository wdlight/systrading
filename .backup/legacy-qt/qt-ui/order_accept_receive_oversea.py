...
"""
  해외 주식 실시간 지연체결가
  TODO : 아직 정리 안함. 1차 국내 코드 베껴옴.
  해외주식 실시간지연 체결가 
  https://apiportal.koreainvestment.com/apiservice-apiservice?/tryitout/HDFSCNT0
...
def receive_realtime_tick_oversea(data):
  """
  메뉴 순서는 '|'로 분리 해서 하나씩 접근함.
  실시간종목코드|종목코드|소수점자리수|현지업업일자|현지일자|현지시간|한국일자|한국시간|시가|고가|저가
  현재가|대비구분|전일대비|등락율|매수호가|매수잔량|매도잔량|체결량|거래량|거래대금|매도체결량|매수체결량|체결강도|시간구분
  """
  values = data.split('^')
  종목코드 = values[0]
  현지시간 = values[5]
      ...
      if data[0] == '0':          
        ...
        if trid0 == "HDFSCNT0": #주식체결 데이터 처리
          ...
          for cnt in range(data_cnt):
            data_dict = receive_realtime_tick_oversea(recvstr[3])
            logger.info(f"주식 체결 데이터: {data_dict}")
            send_data = korea_invest_api.oversea_get_send_data(cmd=4, stock_code=stock_code)
